...
public class WeeklyRankingJobConfig {
	...
	private static final String READER_NAME = "weeklyReader";
	...
	private static final String DELETE_SQL =
		"DELETE FROM product_metrics_weekly WHERE period_key = :periodKey";
	private static final String INSERT_SQL = """
		INSERT INTO product_metrics_weekly (period_key, product_id, ranking, score, like_count, view_count, order_count, created_at, updated_at)
	...
	public Job weeklyRankingJob(Step weeklyRankingStep) {
		log.info("WeeklyRankingJob 배치 등록 완료");
	...
	public Step weeklyRankingStep(
		ItemReader<ProductScoreRow> weeklyReader,
		ItemProcessor<ProductScoreRow, ProductScoreRow> weeklyProcessor,
		ItemWriter<ProductScoreRow> weeklyWriter
	) {
		log.info("주간 랭킹 step 구성 완료 - stepName={}, chunkSize={}", STEP_NAME, CHUNK_SIZE);
	...
	@Bean
	@StepScope
	public ItemReader<ProductScoreRow> weeklyReader(
	...
	@Bean
	@StepScope
	public ItemProcessor<ProductScoreRow, ProductScoreRow> weeklyProcessor() {
		return item -> item;
	}

	@Bean
	@StepScope
	public ItemWriter<ProductScoreRow> weeklyWriter(
		...
		return new ItemWriter<>() {
			...
			public void write(Chunk<? extends ProductScoreRow> chunk) {
				...
				if (dryRun) {
					log.info("[DRY-RUN] 주간 랭킹 저장 예정 - periodKey={}, 건수={}, 순위범위={}..{}",
				...
				if (!deleted) {
					...
					log.info("product_metrics_weekly 선 삭제 완료 - periodKey={}, 삭제행수={}", periodKey, deletedCount);
					deleted = true;
				}
				...
				log.info("product_metrics_weekly 적재 완료 - periodKey={}, 저장행수={}", periodKey, chunk.size());
			}
		};
	}
}
