...
public class OrderServiceTest {
	...
	void createOrder_success() {
		// given
		Long userId = 1L;
		OrderItemRequest request1 = new OrderItemRequest(101L, 2);
		...
		// when
		OrderInfo result = orderService.createOrder(userId, List.of(request1));

		// then
		assertThat(result).isNotNull();
		assertThat(result.userId()).isEqualTo(userId);
		assertThat(result.status()).isEqualTo(OrderStatus.PENDING);
		verify(product1, times(1)).decreaseStock(2);
	...
	void createOrder_insufficientStock_throwsException() {
		// given
		Long userId = 1L;
		OrderItemRequest request1 = new OrderItemRequest(101L, 5);
		Product product1 = mock(Product.class);

		when(productRepository.findById(any())).thenReturn(Optional.of(product1));
		when(product1.getId()).thenReturn(101L);
		// 재고 확인 시 예외를 발생시키도록 설정
		doThrow(new CoreException(null, "재고가 부족합니다.")).when(product1).decreaseStock(anyInt());

		// when and then
		assertThatThrownBy(() -> orderService.createOrder(userId, List.of(request1)))
