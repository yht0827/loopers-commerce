...
public class PointService {
    ...
    @Transactional
    public PointEntity charge(final PointCommand command) {
        PointEntity pointEntity = pointRepository.findByUsersId(command.userId())
                .orElseThrow(() -> new CoreException(ErrorType.NOT_FOUND, " 해당 [id = " + command.userId() + "]의 포인트가 존재하지 않습니다."));

        pointEntity.charge(command.balance());

        return pointEntity;
    }

    @Transactional(readOnly = true)
    public PointEntity getPoint(final Long usersId) {
